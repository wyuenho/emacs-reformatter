...
PROGRAM and ARGS. When DISPLAY-ERRORS is non-nil, shows a buffer
if the formatting fails. For args STDIN, STDOUT, INPUT-FILE,
EXIT-CODE-SUCCESS-P, WORKING-DIRECTORY and OUTPUT-PROCESSOR see
                    ...
                    (let* ((output-file (if stdout stdout-file input-file))
                      ...
                      (if (functionp output-processor)
                          (when (condition-case e
                                    (funcall output-processor output-file success-callback)
                                  (error (with-current-buffer error-buffer
                                           (let ((inhibit-read-only t))
...
;;;###autoload
(cl-defmacro reformatter-define (name &key program args (mode t) (stdin t) (stdout t) input-file lighter keymap group (exit-code-success-p 'zerop) working-directory interactive-modes output-processor)
...
OUTPUT-PROCESSOR
  ...
  If provided, this is a function that takes the output of
  PROGRAM, do some arbitrary processing to it, and then return
  the final output by calling the callback provided as a second
  parameter.  In case of an error, the function should call the
  callback with nil and optionally handle the error in the body.
  If this function is not supplied, the output is returned as is."
